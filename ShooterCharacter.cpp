	...
	bDead(false)
{
	...
	void AShooterCharacter::AbilityEStart()
	{
		if (AbilityESound)
		{
			UGameplayStatics::PlaySound2D(this, AbilityESound);
		}
		...
		if (BarrelSocket)
		{
			...
			if (AbilityEFlash)
			{
				UGameplayStatics::SpawnEmitterAtLocation(GetWorld(), AbilityEFlash, SocketTransform);
			}

			FHitResult AbilityEHit;
			const FVector Start{ SocketTransform.GetLocation() };
			const FQuat Rotation{ SocketTransform.GetRotation() };
			const FVector RotationAxis{ Rotation.GetAxisX() };
			const FVector End{ Start + RotationAxis * 50000.f };

			GetWorld()->LineTraceSingleByChannel(AbilityEHit, Start, End, ECollisionChannel::ECC_Visibility);
			if (AbilityEHit.bBlockingHit)
			{
				DrawDebugLine(GetWorld(), Start, End, FColor::Red, false, 2.f);
				DrawDebugPoint(GetWorld(), AbilityEHit.Location, 5.f, FColor::Red, false, 2.f);

				if (AbilityE_ImpactParticles)
				{
					UGameplayStatics::SpawnEmitterAtLocation(GetWorld(), AbilityE_ImpactParticles, AbilityEHit.Location);
				}
			}
		}
		UAnimInstance* AnimInstance = GetMesh()->GetAnimInstance();
		if (AnimInstance && AbilityEFireMontage)
		{
			AnimInstance->Montage_Play(AbilityEFireMontage);
			AnimInstance->Montage_JumpToSection(FName("StartAbilityE"));
		}
	}
...
void AShooterCharacter::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	PlayerInputComponent->BindAction("AbilityEButton", IE_Pressed, this, &AShooterCharacter::AbilityEStart);
}
